...
function printUppercase(array) {
  ...
  for (let item in array) {
    console.log(array[item].toUpperCase());
  }
}
...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (let item of [[firstElement, ...restElements][1], ...restElements]) {
    if (item < Min) {
      Min = item;
    }
    if (item > Max) {
...
function concatenateStrings(array, subString) {
  ...
  // Виводимо результат
  let list = array.toString();
  console.log(list.includes(subString));
}
...
function replaceNumbers(arr) {
  ...
  for (let item in arr) {
    if (arr[item] > 10) {
      rest.push("Greater than 10");
    }
    if (arr[item] <= 10) {
      rest.push("Less than or equal to 10");
    }
  }
  return rest;
}
